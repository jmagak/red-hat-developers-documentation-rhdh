...
[id="configure-ui-visibility-with-rbac_{context}"]
= Configure UI visibility with RBAC

[role="_abstract"]
You can configure UI visibility for pages, entity tabs, navigation items, and cards by using the `if` predicate in extension blueprints to control which UI elements are visible based on user permissions.

.Prerequisites

* You have administrative access to {product-short}.
* You have enabled the RBAC feature in {product-short}.
...
. Identify the permission names that should gate access to those UI elements.
+
[NOTE]
====
Permissions must follow the naming pattern `<resource>.<action>` where the action is one of `create`, `read`, `update`, or `delete`.
The system automatically infers the action attribute from the permission name.
====
...
. Log in to {product-short} as a user with limited permissions.
...
* xref:ui-visibility-permissions-reference_{context}[UI visibility permissions reference]
