...
[id="configure-telemetry-data-exporters-for-observability-platforms_{context}"]
= Configure telemetry data exporters for observability platforms

[role="_abstract"]
Export trace data to an external observability platform such as Jaeger or an OpenTelemetry Collector. Configure an export strategy and externalize environment variables to ensure reliable telemetry delivery in production.
