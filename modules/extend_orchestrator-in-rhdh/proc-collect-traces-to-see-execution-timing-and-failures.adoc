...
[id="collect-traces-to-see-execution-timing-and-failures_{context}"]
= Collect traces to see execution timing and failures

[role="_abstract"]
Turn on distributed tracing to watch workflows execute step-by-step across services. Traces show exactly which operations run, how long they take, and where failures occur in the execution path.
