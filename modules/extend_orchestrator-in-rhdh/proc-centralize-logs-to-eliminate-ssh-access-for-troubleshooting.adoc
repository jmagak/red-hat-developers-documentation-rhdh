...
[id="centralize-logs-to-eliminate-ssh-access-for-troubleshooting_{context}"]
= Centralize logs to eliminate SSH access for troubleshooting

[role="_abstract"]
Query logs from every workflow pod in your cluster using a single Grafana dashboard. Centralized logging means no more SSHing into individual pods—search by process ID, error type, or time range from your browser.
